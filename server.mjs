...
async function autoScroll(page, maxSteps = 18) {
...
async function ensureGiftsTab(page) {
  ...
  const clicked = await page.evaluate(() => {
    ...
    const el = tabs.find(e => {
      const s = t(e);
      return s.includes('подарк') || s.includes('gift');
    });
...
bot.on('text', async (ctx) => {
  ...
  if (!n || n <= 0) return ctx.reply('Нужно число, например: 1100. Попробуй ещё раз.');
  WAITING.delete(ctx.chat.id);

  const maxStars = n, limit = 15;
  await ctx.reply(`Ищу подарки «за звёзды» до ${maxStars}… Это может занять 10–40 секунд.`);
  try {
...
app.get('/debug/screenshot', async (_req, res) => {
  try {
    const buf = await withBrowser(async (page) => {
      await page.goto(PEEK_URL, { waitUntil: 'networkidle2' });
      await page.waitForTimeout(2500);
      await ensureGiftsTab(page);
